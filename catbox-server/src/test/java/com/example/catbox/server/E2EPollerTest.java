...
/**
 * End-to-End test for the OutboxEventPoller, OutboxEventClaimer, and OutboxEventPublisher.
 * Tests the complete processing loop from event creation to Kafka publication.
 */
...
class E2EPollerTest {
    ...
    /**
     * Gap 1: Tests the core end-to-end poller loop.
     * Verifies that an event, once created, is claimed, published, and marked as sent.
     */
    @Test
    void testPollerClaimsAndPublishesEvent() throws Exception {
        ...
        await()
                ...
                .untilAsserted(() -> {
                    OutboxEvent updatedEvent = outboxEventRepository.findById(savedEvent.getId()).orElseThrow();
