...
/**
 * Configuration for outbox event routing rules.
 * Supports both simple string-based routing (backward compatible) and complex multi-cluster routing.
 */
@Configuration
...
public class OutboxRoutingConfig {
    /**
     * Raw rules map from configuration.
     * Can contain either String values (single cluster) or Map values (multi-cluster config).
     */
    private Map<String, Object> rules = new HashMap<>();
    
    /**
     * Gets the routing rule for a given event type.
     * Handles both old String format and new RoutingRule format.
     * 
    ...
    public RoutingRule getRoutingRule(String eventType) {
        ...
        throw new IllegalArgumentException(
            "Invalid routing rule format for event type: " + eventType + 
            ". Expected String or Map, got: " + ruleConfig.getClass().getName()
        );
    }
