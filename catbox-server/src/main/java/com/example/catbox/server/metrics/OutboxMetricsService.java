...
/**
 * Service that tracks custom metrics for the outbox pattern. Provides insights into outbox health
 * and performance.
 */
@Slf4j
@Service
...
public class OutboxMetricsService {
  ...
  public void initializeMetrics() {
    ...
    eventProcessingTimer =
        Timer.builder("outbox.events.processing.duration")
  ...
  /** Update pending events metrics on a schedule. Runs every 10 seconds to keep metrics current. */
  @Scheduled(
  ...
  public void updatePendingEventsMetrics() {
    try {
      ...
      outboxEventRepository
          .findFirstBySentAtIsNullOrderByCreatedAtAsc()
          .ifPresentOrElse(
              oldestEvent -> {
      ...
      log.debug(
          "Updated outbox metrics: pending={}, oldestAgeSeconds={}",
          pendingCount,
          oldestEventAgeSeconds.get());
    } catch (Exception e) {
  ...
  /**
   * Update archive and dead letter metrics on a schedule. Runs every 30 seconds to keep metrics
   * current.
   */
  @Scheduled(
  ...
  public void updateArchivalMetrics() {
    try {
      ...
      log.debug(
          "Updated archival metrics: archived={}, deadLetter={}", archivedCount, deadLetterCount);
    } catch (Exception e) {
