 ...
 * <ul>
 *   <li>Deduplication using OutboxFilter to prevent duplicate processing</li>
 *   <li>Manual acknowledgment for fine-grained control over message processing</li>
 *   <li>Handling of processing failures with proper error logging</li>
 *   <li>Processing of multiple event types from different topics</li>
 * </ul>
 */
@Slf4j
@Component
...
public class OrderEventListener {
     ...
     * 
     * <p>Uses OutboxFilter to deduplicate messages based on correlation ID.
     * Only processes each unique correlation ID once per consumer group.
     * 
    ...
    public void handleOrderCreated(
        ...
        try {
            ...
            log.error("Processing failed for OrderCreated - correlationId: {}. Message will be retried.", 
                correlationId, e);
            ...
            log.error("Unexpected error processing OrderCreated - correlationId: {}. Acknowledging to continue.", 
                correlationId, e);
            acknowledgment.acknowledge();
        }
    }
    ...
    public void handleOrderStatusChanged(
        ...
        try {
            // Parse the message
            OrderStatusChangedPayload payload = objectMapper.readValue(message, OrderStatusChangedPayload.class);
            ...
            // For intermittent failures, don't acknowledge - message will be retried
            log.error("Processing failed for OrderStatusChanged - correlationId: {}. Message will be retried.", 
                correlationId, e);
            // Note: Not acknowledging will cause Kafka to redeliver the message
            
        } catch (Exception e) {
            // For unexpected errors, log and acknowledge to avoid blocking the queue
            log.error("Unexpected error processing OrderStatusChanged - correlationId: {}. Acknowledging to continue.", 
                correlationId, e);
            acknowledgment.acknowledge();
        }
    }
