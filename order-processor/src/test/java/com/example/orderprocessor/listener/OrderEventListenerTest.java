...
class OrderEventListenerTest {
    ...
    void testHandleOrderCreated_ProcessingException_NotAcknowledged() throws Exception {
        ...
        OrderCreatedPayload payload = new OrderCreatedPayload(
            3L, "Charlie", "Item", new BigDecimal("50.00"), "PENDING"
        );
        String message = objectMapper.writeValueAsString(payload);

        when(outboxFilter.deduped(correlationId, "order-processor")).thenReturn(false);
        doThrow(new OrderEventProcessingService.ProcessingException("Simulated failure"))
            .when(processingService).processOrderCreated(eq(payload), eq(correlationId));
    ...
    void testHandleOrderCreated_NullCorrelationId_ProcessedAndAcknowledged() throws Exception {
        // Given
        OrderCreatedPayload payload = new OrderCreatedPayload(
            4L, "Diana", "Product", new BigDecimal("75.00"), "PENDING"
        );
        String message = objectMapper.writeValueAsString(payload);
    ...
    void testHandleOrderStatusChanged_NewMessage_ProcessedAndAcknowledged() throws Exception {
        ...
        var payload = new com.example.orderprocessor.model.OrderStatusChangedPayload(
            1L, "PENDING", "SHIPPED"
        );
        String message = objectMapper.writeValueAsString(payload);
