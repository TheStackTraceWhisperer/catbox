...
/**
 * Integration test for the DYNAMIC Kafka factory and routing. This test verifies that the factory
...
class KafkaIntegrationTest {
  ...
  static void registerKafkaProperties(DynamicPropertyRegistry registry) {
    ...
    registry.add(
        "kafka.clusters." + TEST_CLUSTER_KEY + ".bootstrap-servers",
        () -> System.getProperty("spring.kafka.bootstrap-servers"));
    registry.add(
        "kafka.clusters." + TEST_CLUSTER_KEY + ".producer.key-serializer",
        () -> "org.apache.kafka.common.serialization.StringSerializer");
    registry.add(
        "kafka.clusters." + TEST_CLUSTER_KEY + ".producer.value-serializer",
        () -> "org.apache.kafka.common.serialization.StringSerializer");
    registry.add("outbox.routing.rules." + TEST_EVENT_TYPE, () -> TEST_CLUSTER_KEY);
  }

  @Autowired private DynamicKafkaTemplateFactory kafkaTemplateFactory;

  @Autowired private OutboxRoutingConfig routingConfig;
  ...
  void testDynamicFactorySendAndReceive() throws Exception {
    ...
    com.example.routebox.server.config.RoutingRule rule =
        routingConfig.getRoutingRule(TEST_EVENT_TYPE);
    ...
    // 5. Verify caching
    KafkaTemplate<String, String> cachedTemplate =
        kafkaTemplateFactory.getTemplate(TEST_CLUSTER_KEY);
