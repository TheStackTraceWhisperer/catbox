package com.example.routebox.client.metrics;
...
class RouteBoxClientMetricsServiceTest {

  private MeterRegistry meterRegistry;
  private RouteBoxClientMetricsService metricsService;

  @BeforeEach
  void setup() {
    meterRegistry = new SimpleMeterRegistry();
    metricsService = new RouteBoxClientMetricsService(meterRegistry);
  ...
  void metricsAreRegistered() {
    ...
    Counter outboxWriteSuccessCounter =
        meterRegistry.find("routebox.client.outbox.write.success").counter();
    assertThat(outboxWriteSuccessCounter).isNotNull();

    Counter outboxWriteFailureCounter =
        meterRegistry.find("routebox.client.outbox.write.failure").counter();
    assertThat(outboxWriteFailureCounter).isNotNull();

    Counter filterUniqueCounter = meterRegistry.find("routebox.client.filter.unique").counter();
    assertThat(filterUniqueCounter).isNotNull();

    Counter filterDedupedCounter = meterRegistry.find("routebox.client.filter.deduped").counter();
    assertThat(filterDedupedCounter).isNotNull();

    Counter filterConcurrentDuplicateCounter =
        meterRegistry.find("routebox.client.filter.concurrent.duplicate").counter();
    assertThat(filterConcurrentDuplicateCounter).isNotNull();

    Counter filterMarkProcessedCounter =
        meterRegistry.find("routebox.client.filter.mark.processed").counter();
    assertThat(filterMarkProcessedCounter).isNotNull();

    Counter filterMarkUnprocessedCounter =
        meterRegistry.find("routebox.client.filter.mark.unprocessed").counter();
    assertThat(filterMarkUnprocessedCounter).isNotNull();
  }

  @Test
  void recordOutboxWriteSuccess_incrementsCounter() {
    Counter counter = meterRegistry.find("routebox.client.outbox.write.success").counter();
  ...
  void recordOutboxWriteFailure_incrementsCounter() {
    Counter counter = meterRegistry.find("routebox.client.outbox.write.failure").counter();
    double initialCount = counter.count();
  ...
  void recordFilterUnique_incrementsCounter() {
    Counter counter = meterRegistry.find("routebox.client.filter.unique").counter();
    double initialCount = counter.count();
  ...
  void recordFilterDeduped_incrementsCounter() {
    Counter counter = meterRegistry.find("routebox.client.filter.deduped").counter();
    double initialCount = counter.count();
  ...
  void recordFilterConcurrentDuplicate_incrementsCounter() {
    Counter counter = meterRegistry.find("routebox.client.filter.concurrent.duplicate").counter();
    double initialCount = counter.count();
  ...
  void recordFilterMarkProcessed_incrementsCounter() {
    Counter counter = meterRegistry.find("routebox.client.filter.mark.processed").counter();
    double initialCount = counter.count();
  ...
  void recordFilterMarkUnprocessed_incrementsCounter() {
    Counter counter = meterRegistry.find("routebox.client.filter.mark.unprocessed").counter();
    double initialCount = counter.count();
