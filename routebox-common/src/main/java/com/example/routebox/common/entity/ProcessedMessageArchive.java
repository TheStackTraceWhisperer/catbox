...
/**
 * Entity representing archived processed messages.
 * Messages are moved here after a retention period to prevent unbounded
 * table growth.
 */
@Entity
@Table(name = "processed_messages_archive",
        indexes = {
            @Index(name = "idx_archive_correlation_consumer",
                   columnList = "correlationId,consumerGroup"),
            @Index(name = "idx_archive_archived_at",
                   columnList = "archivedAt")
        })
@Getter
@Setter
@NoArgsConstructor
