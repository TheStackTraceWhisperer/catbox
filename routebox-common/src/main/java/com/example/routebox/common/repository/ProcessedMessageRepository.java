...
public interface ProcessedMessageRepository extends JpaRepository<ProcessedMessage, Long> {
   ...
   */
  boolean existsByCorrelationIdAndConsumerGroup(String correlationId, String consumerGroup);
  ...
  Optional<ProcessedMessage> findByCorrelationIdAndConsumerGroup(
      String correlationId, String consumerGroup);
  ...
  @Modifying
  @Query(
      "DELETE FROM ProcessedMessage p WHERE p.correlationId = :correlationId "
          + "AND p.consumerGroup = :consumerGroup")
  void deleteByCorrelationIdAndConsumerGroup(
      @Param("correlationId") String correlationId, @Param("consumerGroup") String consumerGroup);

  /**
   * Delete processed messages older than the specified date. Used for archival cleanup.
   *
